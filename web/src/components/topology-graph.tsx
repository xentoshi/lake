  ...
  const linkInfoMap = useMemo(() => {
    ...
    for (const link of topologyData.links) {
      map.set(link.pk, {
        ...
        latencyAtoZMs: link.latency_a_to_z_us ? `${(link.latency_a_to_z_us / 1000).toFixed(2)}ms` : 'N/A',
        jitterAtoZMs: link.jitter_a_to_z_us ? `${(link.jitter_a_to_z_us / 1000).toFixed(2)}ms` : 'N/A',
        latencyZtoAMs: link.latency_z_to_a_us ? `${(link.latency_z_to_a_us / 1000).toFixed(2)}ms` : 'N/A',
        jitterZtoAMs: link.jitter_z_to_a_us ? `${(link.jitter_z_to_a_us / 1000).toFixed(2)}ms` : 'N/A',
